...
app = FastAPI(
    title="MiniRobot BLE Web API",
    description="REST API for controlling MiniRobot balancing car via BLE",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def set_light(request: LightRequest):
    val = 0
    if request.headlight:
        val |= 0x01
    if request.brakelight:
        val |= 0x02
    if request.lock_shutdown:
        val |= 0x04
    if request.lock_warn:
        val |= 0x08
    if request.back_alarm:
        val |= 0x10
    if request.carlight:
        val |= 0x80
